...
function navigate(section, el) {
    ...
    const titles = { dashboard: 'Dashboard', plan: 'Plan 30 días', habitos: 'Hábitos', clientes: 'Clientes', finanzas: 'Finanzas', ideas: 'Ideas', metricas: 'Métricas IG', comparar: 'Comparar', servicios: 'Servicios', mensajes: 'Mensajes', testimonios: 'Testimonios' };
...
function renderDashboard() {
    ...
    if (dayEl) {
        if (dayNum > 30) {
            dayEl.textContent = `Plan de 30 días completado`;
        } else {
            const restan = 30 - dayNum;
            dayEl.textContent = `Día ${dayNum} de 30 · quedan ${restan} día${restan === 1 ? '' : 's'}`;
        }
    }
